...
function ReviewPage() {
  ...
  const [session, setSession] = useState<{ task_title: string; duration_seconds: number; todo_id: string | null } | null>(null);
  ...
  useEffect(() => {
    (async () => {
      const { data, error } = await supabase
        ...
        .eq("id", sessionId)
        .single();
      if (error || !data) { toast.error("Session not found"); navigate({ to: "/app" }); return; }
      setSession(data);
    })();
  }, [sessionId, navigate]);
  ...
  const finalize = async (completeTask: boolean) => {
    if (!session) return;
    setBusy(true);
    const { error } = await supabase.from("study_sessions").update({ saved: true }).eq("id", sessionId);
      ...
      <div className="mx-auto max-w-md">
        <Card className="p-8 text-center">
          ...
          <div className="mt-8 rounded-xl bg-muted p-6">
            <p className="text-xs uppercase tracking-wider text-muted-foreground">Total focus time</p>
            <p className="mt-1 font-mono text-4xl font-light tabular-nums">{fmt(session.duration_seconds)}</p>
          </div>
          ...
          <div className="mt-8 flex flex-col gap-2">
            ...
            <Button onClick={discard} disabled={busy} variant="ghost" className="gap-2 text-destructive hover:text-destructive">
